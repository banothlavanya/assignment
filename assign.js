...
request2.open('GET', 'data1.json', true);
  ...
  if (request2.status >= 200 && request2.status < 400){
   container.querySelectorAll('*').forEach(n => n.remove());
      ...
      Courses.data.forEach(course => {
       ...
       if(instructor_name_array.includes(searchQuery) || title_array.includes(searchQuery)){
...
xmlhttp.onreadystatechange = function accesingValues() {
 ...
 z+='<input type="search" name="Category" onkeyup="searchbar()"+ placeholder="filter"/>'+ "<br>";
